...
func abortWithTokenAuthError(c *gin.Context, err error) {
	switch {
	case errors.Is(err, model.ErrTokenNotProvided):
		abortWithOpenAiMessage(c, http.StatusUnauthorized, "API key is required", types.ErrorCodeInvalidToken)
	case errors.Is(err, model.ErrTokenDisabled):
		abortWithOpenAiMessage(c, http.StatusForbidden, "API key is disabled", types.ErrorCodeTokenDisabled)
	case errors.Is(err, model.ErrTokenExpired):
		abortWithOpenAiMessage(c, http.StatusForbidden, "API key has expired", types.ErrorCodeTokenExpired)
	case errors.Is(err, model.ErrTokenExhausted):
		abortWithOpenAiMessage(c, http.StatusTooManyRequests, "API key quota exhausted", types.ErrorCodeTokenExhausted)
	default:
		abortWithOpenAiMessage(c, http.StatusUnauthorized,
			common.TranslateMessage(c, i18n.MsgTokenInvalid), types.ErrorCodeInvalidToken)
	}
}
