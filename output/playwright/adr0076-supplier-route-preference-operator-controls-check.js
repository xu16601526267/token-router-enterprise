...
const desktopScreenshot =
  'output/playwright/adr0076-supplier-route-preference-operator-controls.png'
const mobileScreenshot =
  'output/playwright/adr0076-supplier-route-preference-operator-controls-mobile.png'
...
const fulfillJson = async (route, body) => {
  ...
  await page.route('**/api/**', async (route) => {
    ...
    if (
      path === '/api/supplier_route_preferences/activate' &&
      method === 'POST'
    ) {
      const input = JSON.parse(postData || '{}')
      assert(input.supplier_id === supplier.id, 'activate supplier_id drifted')
      assert(input.weight_percent === 60, 'activate weight_percent drifted')
      assert(
        input.reason === 'operator manual route preference in playwright',
    ...
    if (
      path === `/api/supplier_route_preferences/${supplier.id}/disable` &&
      method === 'POST'
    ) {
      const input = JSON.parse(postData || '{}')
      assert(
  ...
  await dialog.getByLabel('Route Weight Percent').fill('60')
  await dialog
    .getByLabel('Reason')
    .fill('operator manual route preference in playwright')
  await dialog.getByLabel('Operator Note').fill('playwright manual watch')
  await dialog
    .getByRole('button', { name: 'Set Route Preference' })
    .click()
  ...
  await waitForRequest(
    ...
    (entry) =>
      entry.method === 'POST' &&
      entry.path === '/api/supplier_route_preferences/activate',
    'activate POST was not observed after submitting route preference'
  )
  await page.getByText('Manual', { exact: true }).first().waitFor()
  await page.getByText('60%').first().waitFor()
  await page
    .getByText('operator manual route preference in playwright')
    .first()
    .waitFor()
