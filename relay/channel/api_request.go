...
func tokenRouterUpstreamSessionID(c *gin.Context, info *common.RelayInfo) string {
	headerKeys := []string{"X-Session-Id", "X-Session-ID", "session_id", "X-Prompt-Cache-Key"}
	if info != nil {
...
func applyTokenRouterSessionHeaders(headers http.Header, c *gin.Context, info *common.RelayInfo) {
	sessionID := tokenRouterUpstreamSessionID(c, info)
	if sessionID == "" {
		return
	}
	if strings.TrimSpace(headers.Get("X-Session-Id")) == "" {
...
func DoApiRequest(a Adaptor, c *gin.Context, info *common.RelayInfo, requestBody io.Reader) (*http.Response, error) {
	...
	applyTokenRouterSessionHeaders(headers, c, info)
...
func DoWssRequest(a Adaptor, c *gin.Context, info *common.RelayInfo, requestBody io.Reader) (*websocket.Conn, error) {
	...
	applyTokenRouterSessionHeaders(targetHeader, c, info)
	// 在 SetupRequestHeader 之后应用 Header Override，确保用户设置优先级最高
	// 这样可以覆盖默认的 Authorization header 设置
	headerOverride, err := processHeaderOverride(info, c)
	if err != nil {
