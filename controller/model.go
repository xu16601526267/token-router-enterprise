...
type userVisibleModels struct {
	models []dto.OpenAIModels
	byID   map[string]dto.OpenAIModels
}
...
func getUserVisibleModels(c *gin.Context) (userVisibleModels, error) {
	...
	userOpenAiModelsByID := make(map[string]dto.OpenAIModels, len(userModelNames))
	for _, modelName := range userModelNames {
		if _, exists := userOpenAiModelsByID[modelName]; exists {
			continue
		}
		oaiModel := buildOpenAIModel(modelName, ownerByModel)
		userOpenAiModels = append(userOpenAiModels, oaiModel)
		userOpenAiModelsByID[modelName] = oaiModel
	}

	return userVisibleModels{
		models: userOpenAiModels,
		byID:   userOpenAiModelsByID,
	}, nil
}

func ListModels(c *gin.Context, modelType int) {
	visibleModels, err := getUserVisibleModels(c)
	if err != nil {
	...
	case constant.ChannelTypeAnthropic:
		useranthropicModels := make([]dto.AnthropicModel, len(visibleModels.models))
		for i, model := range visibleModels.models {
	...
	case constant.ChannelTypeGemini:
		userGeminiModels := make([]dto.GeminiModel, len(visibleModels.models))
		for i, model := range visibleModels.models {
...
func RetrieveModel(c *gin.Context, modelType int) {
	...
	if aiModel, ok := visibleModels.byID[modelId]; ok {
		switch modelType {
		case constant.ChannelTypeAnthropic:
