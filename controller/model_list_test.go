...
func TestRetrieveModelUsesUserVisibleModels(t *testing.T) {
	withSelfUseModeDisabled(t)
	withTieredBillingConfig(t, map[string]string{
		"zz-retrieve-visible-model": "tiered_expr",
	}, map[string]string{
		"zz-retrieve-visible-model": `tier("base", p + c)`,
	})
	...
	require.NoError(t, db.Create(&model.Ability{
		Group:     "default",
		Model:     "zz-retrieve-visible-model",
		ChannelId: 1,
		Enabled:   true,
	}).Error)
	...
	recorder := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(recorder)
	ctx.Request = httptest.NewRequest(http.MethodGet, "/v1/models/zz-retrieve-visible-model", nil)
	ctx.Params = gin.Params{{Key: "model", Value: "zz-retrieve-visible-model"}}
	ctx.Set("id", 1002)
	...
	require.NoError(t, common.Unmarshal(recorder.Body.Bytes(), &payload))
	require.Equal(t, "zz-retrieve-visible-model", payload.Id)
	require.Equal(t, "model", payload.Object)
	require.Equal(t, "custom", payload.OwnedBy)
}

func TestRetrieveModelRejectsInvisibleModel(t *testing.T) {
	withSelfUseModeDisabled(t)
	withTieredBillingConfig(t, map[string]string{
		"zz-retrieve-hidden-model": "tiered_expr",
	}, map[string]string{
		"zz-retrieve-hidden-model": `tier("base", p + c)`,
	})

	db := setupModelListControllerTestDB(t)
	require.NoError(t, db.Create(&model.User{
		Id:       1003,
		Username: "model-hidden-user",
		Password: "password",
		Group:    "default",
		Status:   common.UserStatusEnabled,
	}).Error)
	...
	recorder := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(recorder)
	ctx.Request = httptest.NewRequest(http.MethodGet, "/v1/models/zz-retrieve-hidden-model", nil)
	ctx.Params = gin.Params{{Key: "model", Value: "zz-retrieve-hidden-model"}}
	ctx.Set("id", 1003)
