...
type tokenUsageAPIResponse struct {
	...
	Data    struct {
		...
		TokenTotalGranted   int    `json:"token_total_granted"`
		TokenTotalUsed      int    `json:"token_total_used"`
		TokenTotalAvailable int    `json:"token_total_available"`
		UserTotalAvailable  int    `json:"user_total_available"`
		EffectiveAvailable  int    `json:"effective_available"`
		UserQuotaLimited    bool   `json:"user_quota_limited"`
...
func TestGetTokenUsageIncludesUserAndEffectiveQuota(t *testing.T) {
	db := openTokenControllerTestDB(t)
	if err := db.AutoMigrate(&model.User{}, &model.Token{}); err != nil {
		t.Fatalf("failed to migrate token usage tables: %v", err)
	}
	...
	ctx.Request.Header.Set("Authorization", "Bearer sk-usage-key")
	...
	var payload tokenUsageAPIResponse
	if err := common.Unmarshal(recorder.Body.Bytes(), &payload); err != nil {
		t.Fatalf("failed to decode token usage response: %v", err)
	}
	if !payload.Code {
		t.Fatalf("expected code=true, got response: %s", recorder.Body.String())
	}
	if payload.Data.TotalAvailable != 100 || payload.Data.TokenTotalAvailable != 100 {
		t.Fatalf("expected token available quota 100, got %+v", payload.Data)
	}
	if payload.Data.UserTotalAvailable != 75 || payload.Data.EffectiveAvailable != 75 {
		t.Fatalf("expected user/effective quota 75, got %+v", payload.Data)
	}
	if !payload.Data.UserQuotaLimited {
		t.Fatalf("expected user_quota_limited=true, got %+v", payload.Data)
	}
}

func TestValidateUserTokenReturnsSpecificStatusErrors(t *testing.T) {
	db := setupTokenControllerTestDB(t)
	...
	cases := []struct {
	...
	}{
		{
			name: "disabled",
			token: model.Token{
				UserId: 1, Name: "disabled-token", Key: "disabled-key", Status: common.TokenStatusDisabled,
				CreatedTime: 1, AccessedTime: 1, ExpiredTime: -1, RemainQuota: 100,
			},
			wantError: model.ErrTokenDisabled,
		},
		{
			name: "expired status",
			token: model.Token{
				UserId: 1, Name: "expired-status-token", Key: "expired-status-key", Status: common.TokenStatusExpired,
				CreatedTime: 1, AccessedTime: 1, ExpiredTime: -1, RemainQuota: 100,
			},
			wantError: model.ErrTokenExpired,
		},
		{
			name: "expired time",
			token: model.Token{
				UserId: 1, Name: "expired-time-token", Key: "expired-time-key", Status: common.TokenStatusEnabled,
				CreatedTime: 1, AccessedTime: 1, ExpiredTime: now - 1, RemainQuota: 100,
			},
			wantError: model.ErrTokenExpired,
		},
		{
			...
			token: model.Token{
				UserId: 1, Name: "exhausted-status-token", Key: "exhausted-status-key", Status: common.TokenStatusExhausted,
				CreatedTime: 1, AccessedTime: 1, ExpiredTime: -1, RemainQuota: 100,
			},
			wantError: model.ErrTokenExhausted,
		},
		{
			name: "exhausted quota",
			token: model.Token{
				UserId: 1, Name: "exhausted-quota-token", Key: "exhausted-quota-key", Status: common.TokenStatusEnabled,
				CreatedTime: 1, AccessedTime: 1, ExpiredTime: -1, RemainQuota: 0,
			},
			wantError: model.ErrTokenExhausted,
		},
	}
